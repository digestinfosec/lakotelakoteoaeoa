...
class Googlemaps {
	...
	function add_marker($params = array())
	{
		...
		$marker['div_id'] = '';									// For multiple map
		...
		if($marker['div_id']!="") {
			$this->map_div_id = $marker['div_id'];
			$this->markersInfo['marker_'.$marker_id]['div_id'] = $marker['div_id'];
		}
        else $this->map_div_id = $this->map_name;		

		if ($marker['position']!="") {
			if ($this->is_lat_long($marker['position'])) {
				$marker_output .= '
			var myLatlng_'.$this->map_div_id.' = new google.maps.LatLng('.$marker['position'].');
			';
				...
				$marker_output .= '
			var myLatlng_'.$this->map_div_id.' = new google.maps.LatLng('.$lat_long[0].', '.$lat_long[1].');';
		...
		if ($marker['icon']!="") {
			$marker_output .= '
				var marker_icon_'.$this->map_div_id.' = {
					url: "'.$marker['icon'].'"';
			if ($marker['icon_size']!="") {
				$marker_output .= ',
		...
		$marker_output .= '	
			var markerOptions_'.$this->map_div_id.' = {
				map: '.$this->map_div_id;
		if ($marker['position']!="") {
			$marker_output .= ',
				position: myLatlng_'.$this->map_div_id;
		}
		if (!$marker['clickable']) {
			$marker_output .= ',
				clickable: false';
		...
		if ($marker['icon']!="") {
			$marker_output .= ',
				icon: marker_icon_'.$this->map_div_id;
		}
		...
		$marker_output .= '		
			...
			marker_'.$marker_id.' = createMarker_'.$this->map_div_id.'(markerOptions_'.$this->map_div_id.');
			';
		...
		if ($marker['infowindow_content']!="") {
			...
			google.maps.event.addListener(marker_'.$marker_id.', "click", function(event) {
				iw_'.$this->map_div_id.'.setContent(this.get("content"));
				iw_'.$this->map_div_id.'.open('.$this->map_div_id.', this);
			';
    ...
    {
        if ($div_id) $this->map_div_id = $div_id;
        else $this->map_div_id = $map_name;
        $this->output_js = '';
        ...
        $this->output_js_contents .= '
            var '.$this->map_div_id.'; // Global declaration of the map
            var lat_longs_'.$this->map_div_id.' = new Array();
            var markers_'.$this->map_div_id.' = new Array();
            var iw_'.$this->map_div_id.';
            ';
        if ($this->cluster) {
        ...
        $this->output_js_contents .= '
            iw_'.$this->map_div_id.' = new google.maps.InfoWindow(';
        ...
        $this->output_js_contents .= 'function initialize_'.$this->map_div_id.'() {
        ...
        if (count($this->styles)) {
            ...
            foreach ($this->styles as $style) {
                ...
                ';
                    ...
                    $styleOutput .= $this->map_div_id.'.setOptions({styles: styles_'.$styles.'});
                ';
                    break;
                }
        ...
        if (count($this->styles) && $this->stylesAsMapTypes) {
            ...
            foreach ($this->styles as $style) {
                ...
                $styleOutput .= '
                      '.$this->map_div_id.'.mapTypes.set("style'.$styles.'", styles_'.$styles.');
                ';
                if ($this->stylesAsMapTypesDefault==$style['name']) {
                $styleOutput .= '
                      '.$this->map_div_id.'.setMapTypeId("style'.$styles.'");
                ';
                }
                $styles++;
            }
        ...
        $this->output_js_contents .= '}
                '.$this->map_div_id.' = new google.maps.Map(document.getElementById("'.$this->map_div_id.'"), myOptions);
                ';
        ...
        if ($this->panoramio) {
            ...
            if ($this->panoramioUser!="") { $this->output_js_contents .= 'panoramioLayer.setUserId("'.$this->panoramioUser.'");
                '; }
            $this->output_js_contents .= '
                panoramioLayer.setMap('.$this->map_div_id.');
                ';
        }
            ...
            $this->output_js_contents .= '
                ...
                if(navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                        '.$this->map_div_id.'.setCenter(new google.maps.LatLng(position.coords.latitude,position.coords.longitude));
        ...
        if ($this->directions) {
            $this->output_js_contents .= 'directionsDisplay.setMap('.$this->map_div_id.');
            ';
        ...
        if ($this->drawing) {
            ...
            drawingManager.setMap('.$this->map_div_id.');
            ';
        ...
        if ($this->places) {
            ...
            if (($placesLocationSet || $this->placesLocation!="") || count($this->placesTypes) || $this->placesName!="")
            {
                ...
                placesService = new google.maps.places.PlacesService('.$this->map_div_id.');
        ...
        if ($this->onboundschanged!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "bounds_changed", function(event) {
                '.$this->onboundschanged.'
            });
            ';
        }
        if ($this->oncenterchanged!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "center_changed", function(event) {
                '.$this->oncenterchanged.'
            });
            ';
        }
        if ($this->onclick!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "click", function(event) {
                '.$this->onclick.'
            });
            ';
        }
        if ($this->ondblclick!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "dblclick", function(event) {
                '.$this->ondblclick.'
            });
            ';
        }
        if ($this->ondrag!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "drag", function(event) {
                '.$this->ondrag.'
            });
            ';
        }
        if ($this->ondragend!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "dragend", function(event) {
                '.$this->ondragend.'
            });
            ';
        }
        if ($this->ondragstart!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "dragstart", function(event) {
                '.$this->ondragstart.'
            });
            ';
        }
        if ($this->onidle!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "idle", function(event) {
                '.$this->onidle.'
            });
            ';
        }
        if ($this->onmousemove!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "mousemove", function(event) {
                '.$this->onmousemove.'
            });
            ';
        }
        if ($this->onmouseout!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "mouseout", function(event) {
                '.$this->onmouseout.'
            });
            ';
        }
        if ($this->onmouseover!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "mouseover", function(event) {
                '.$this->onmouseover.'
            });
            ';
        }
        if ($this->onresize!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "resize", function(event) {
                '.$this->onresize.'
            });
            ';
        }
        if ($this->onrightclick!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "rightclick", function(event) {
                '.$this->onrightclick.'
            });
            ';
        }
        if ($this->ontilesloaded!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "tilesloaded", function(event) {
                '.$this->ontilesloaded.'
            });
            ';
        }
        if ($this->onzoomchanged!="") {
            $this->output_js_contents .= 'google.maps.event.addListener('.$this->map_div_id.', "zoom_changed", function(event) {
                '.$this->onzoomchanged.'
            });
            ';
        }
        ...
        if (count($this->markers)) {
            foreach ($this->markers as $key=>$marker) {
            	if($this->markersInfo['marker_'.$key]['div_id'] != "" && 
            		$this->markersInfo['marker_'.$key]['div_id'] != $this->map_div_id) {
            		continue;
            	}
        ...
        if ($this->cluster) {
            ...
            markerCluster = new MarkerClusterer('.$this->map_div_id.', markers_'.$this->map_div_id.', clusterOptions);
            ';
        }
        ...
        if ($this->zoom=="auto") {
            ...
            $this->output_js_contents .= '
            fitMapToBounds_'.$this->map_div_id.'();
            ';
        ...
        if ($this->adsense) {
            ...
            if ($this->adsenseChannelNumber!="") { $this->output_js_contents .= 'channelNumber: "'.$this->adsenseChannelNumber.'",
                '; }
            $this->output_js_contents .= 'map: '.$this->map_div_id.',
                visible: true
            };
        ...
        // add markers
        $this->output_js_contents .= '
        function createMarker_'.$this->map_div_id.'(markerOptions_'.$this->map_div_id.') {
            var marker = new google.maps.Marker(markerOptions_'.$this->map_div_id.');
            markers_'.$this->map_div_id.'.push(marker);
            lat_longs_'.$this->map_div_id.'.push(marker.getPosition());
            return marker;
        }
        ';
        //
            ...
            $this->output_js_contents .= 'function placesCallback(results, status) {
                if (status == google.maps.places.PlacesServiceStatus.OK) {
                    for (var i = 0; i < results.length; i++) {
                        ...
                        var marker = createMarker_'.$this->map_div_id.'(markerOptions);
                        ...
                        lat_longs_'.$this->map_div_id.'.push(placePosition);
                    
                    }
                    ';
            if ($this->zoom=="auto") { $this->output_js_contents .= 'fitMapToBounds_'.$this->map_div_id.'();'; }
            $this->output_js_contents .= '
                }
            }
            ';
        ...
        if ($this->zoom=="auto") {
            $this->output_js_contents .= '
            function fitMapToBounds_'.$this->map_div_id.'() {
                var bounds = new google.maps.LatLngBounds();
                if (lat_longs_'.$this->map_div_id.'.length>0) {
                    for (var i=0; i<lat_longs_'.$this->map_div_id.'.length; i++) {
                        bounds.extend(lat_longs_'.$this->map_div_id.'[i]);
                    }
                    '.$this->map_div_id.'.fitBounds(bounds);
                }
            }
            ';
        }
        ...
        if ($this->loadAsynchronously) {
            $this->output_js_contents .= '
            function loadScript_'.$this->map_div_id.'() {
                var script = document.createElement("script");
                script.type = "text/javascript";
                script.src = "'.$apiLocation.'&callback=initialize_'.$this->map_div_id.'";
                document.body.appendChild(script);
            }
            window.onload = loadScript_'.$this->map_div_id.';
            ';
        }else{
            $this->output_js_contents .= '
            google.maps.event.addDomListener(window, "load", initialize_'.$this->map_div_id.');
            ';
        }
